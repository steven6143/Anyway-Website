...
function convert_ubb ($str, $advanced=0, $inrss=0) {
	...
	//[IMG]
	if ($advanced==1) {
		$str=preg_replace("/\[url=([^\[]*)\]\[img( align=L| align=M| align=R)?( width=[0-9]+)?( height=[0-9]+)?( label=[\s\S]+?)?\]\s*(\S+?)\s*\[\/img\]\[\/url\]/ise","makeimgwithurl('\\1', '\\2', '\\3', '\\4', '\\5', '\\6', {$inrss})",$str);
		$str=preg_replace("/\[img( align=L| align=M| align=R)?( width=[0-9]+)?( height=[0-9]+)?( label=[\s\S]+?)?\]\s*(\S+?)\s*\[\/img\]/ise","makeimg('\\1', '\\2', '\\3', '\\4', '\\5', {$inrss})",$str);
	} else {
...
function makeurl($url) {
	global $mbcon;
	$urllink="<a href=\"".(substr(strtolower($url), 0, 4) == 'www.' ? "http://$url" : $url).'">';
...
function makeimg ($aligncode, $widthcode, $heightcode, $labelcode, $src, $inrss=0) {
	...
	$label=str_replace(' label=', '', $labelcode);
	if (!empty($label)) $imagelabel="<div class='image-label'>{$label}</div>";	
	$code="<img src=\"{$src}\" class=\"insertimage png-bg\" alt=\"图片载入中...\" title=\"\" border=\"0\"{$onloadact}{$show}/>{$imagelabel}";
	return $code;
}

function makeimgwithurl ($url, $aligncode, $widthcode, $heightcode, $labelcode, $src, $inrss=0) {
	global $lnc, $mbcon, $config;
	$align=str_replace(' align=', '', strtolower($aligncode));
	if ($align=='l') $show=' align="left"';
	elseif ($align=='r') $show=' align="right"';
	...
	$label=str_replace(' label=', '', $labelcode);
	if (!empty($label)) $imagelabel="<br />{$label}";	
	
	if ($inrss==1) $src=(substr(strtolower($src), 0, 4) == 'http') ? $src : $config['blogurl'].'/'.$src;
	$onloadact=($inrss==0 && !empty($mbcon['autoresizeimg'])) ? " onload=\"if(this.width>{$mbcon['autoresizeimg']}) {this.resized=true; this.width={$mbcon['autoresizeimg']};}\"" : '';
	$code="<a href=\"{$url}\"><img src=\"{$src}\" class=\"insertimage\" alt=\"图片载入中...\" title=\"\" border=\"0\"{$onloadact}{$show}/></a>{$imagelabel}";
	return $code;
}
...
function makeimginrss($src) {
	...
	$str="<br/><img src=\"{$config['blogurl']}/{$template['images']}/viewimage.gif\" alt=\"321123\"/><a href=\"{$src}\" >{$lnc[231]}</a><br/>[url]{$src}[/url]<br/>";
	return $str;
}
