...
class getblogs extends boblog {
	...
	function single_reply ($eachreply, $i=0) {
		...
		if ($eachreply['reproperty']==0 || $eachreply['reproperty']==1) { // A Normal Reply
			...
			if ($eachreply['replierid']==-1) {
				...
				$replier="<a href=\"".getlink_user($eachreply['replierid'])."\" title=\"{$lnc[17]}\">{$eachreply['replier']}</a>";
				//Avatars for users
				if ($flset['avatar']!=1) {
			...
			if ($permission['SeeIP']==1) $replierip="<a href=\"{$mbcon['ipsearch']}{$eachreply['repip']}\" ><img src=\"{$mbcon['images']}/ip.gif\" border=\"0\" alt=\"IP\" title=\"IP: {$eachreply['repip']}\" /></a>";
			...
			if ($eachreply['repurl']) $replierhomepage="<a href=\"{$eachreply['repurl']}\" ><img src=\"{$mbcon['images']}/homepage.gif\" border=\"0\" alt=\"Homepage\" title=\"{$lnc[19]}\" /></a>";
			...
			if ($eachreply['adminreptime'] && ($eachreply['reproperty']!=1 || $permission['SeeHiddenReply']==1 || $eachreply['reppsw']=='')) {
				...
				$adminreplier="<a href=\"".getlink_user($eachreply['adminrepid'])."\" title=\"{$lnc[17]}\">{$eachreply['adminreplier']}</a>";
	...
	function output ($entry, $way='excerpt', $contentonly=false) {
		...
		$tdbfy0am = strtotime(date('Y-m-d',strtotime('-2 day',$time0)));  //获取前天凌晨的时间戳
		$time1 = $entry['pubtime']+3600*$config['timezone'];  //帖子发布时间
		$tc = time() - $entry['pubtime'];  //相对时间间隔
		...
		if ($entry['property']==1) {//Locked
			$entrycomment="$lnc[36]";
		} else $entrycomment="<a href=\"".getlink_entry($entry['blogid'], $entry['blogalias'])."#reply\" title=\"{$lnc[37]}\">{$entry['replies']} $lnc[38]</a>";
